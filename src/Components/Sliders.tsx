...
const checkIfImageExists = async (imageUrl: string) => {
			...
			<AnimatePresence onExitComplete={toggleSliding}>
				...
				>
					{movies
						...
						.map((movie) => {
							var imagePath = movie.name ?? movie.title;
							imagePath = `${imagePath?.replace(/\s/g, '')}.jpg`;
							const exists =  checkIfImageExists(imagePath);
							if(!exists)	imagePath = makeImagePath(movie.backdrop_path ?? movie.poster_path, "w300");
							return (
							<Box
								key={movie.id}
								...
								bgphoto={imagePath || makeImagePath(movie.backdrop_path ?? movie.poster_path, "w300")}
